...
// Maps vowels to replacement words
static const struct {
    char ch;
    const char *word;
} vowel_map[] = {
...
int getchar(void) {
    if (!orig_getchar) {
        orig_getchar = (orig_getchar_type)dlsym(RTLD_NEXT, "getchar");
        if (!orig_getchar) {
    ...
    // Return next character from replacement string
    ...
    // Check if character is a vowel to replace
